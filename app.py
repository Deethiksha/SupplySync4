...
# ============================================================
    ...
    truck_display = truck_profile.rename(columns={
        VEHICLE_COL: "Vehicle", "capacity_tons": "Capacity (tons)", "stores_served": "Stores Served", "role": "Type"
    })[["Vehicle", "Type", "Capacity (tons)", "Stores Served"]].copy()
    ...
    if st.session_state.get("fleet_allocation_v2_run", False):
        ...
        st.markdown(
            f"""<div style="background-color:#cfe0c8;border-radius:6px;padding:10px 16px;
                ...
                Every dispatched trip is loaded to at least {full_threshold_pct}% of that
                truck's real capacity, and stores with light loads share a trip instead of each
                triggering a new one. Nothing dispatches half-empty.
            </div>""",
            unsafe_allow_html=True
        )
        ...
        allocation_table = pd.DataFrame(rows).sort_values(["Vehicle", "Trip #"]).reset_index(drop=True) \
            if rows else pd.DataFrame(columns=["Vehicle", "Trip #", "Type", "Stores Served", "Stops", "Load (tons)", "Capacity (tons)", "Utilization %"])
        ...
        grid_cards([
            ...
            ("Trips Dispatched", f"{optimized_total}"),
            ("Avg Utilization (dispatched)", f"{avg_util:.1f}%"),
            ("Held-Back Demand", f"{held_tons:.2f} tons"),
        ])
        ...
        elif avg_util >= full_threshold_pct:
        ...
        if dispatched_trips:
            ...
            # ============================================================
            # VISUAL 3 — demand vs trips dispatched, by vehicle
            # ============================================================
            st.markdown("<div style='margin-top:18px'></div>", unsafe_allow_html=True)
            st.markdown("<h4 style='color:black;'>Demand Carried (tons) vs Trips Dispatched, by Vehicle</h4>", unsafe_allow_html=True)

            by_vehicle = allocation_table.groupby("Vehicle").agg(
                total_load_tons=("Load (tons)", "sum"), trips=("Trip #", "nunique")
            ).reindex(truck_profile[VEHICLE_COL]).fillna(0)

            fig3, ax3 = plt.subplots(figsize=(9, 5))
            x = np.arange(len(by_vehicle))
            ax3b = ax3.twinx()
            ax3.bar(x - 0.2, by_vehicle["total_load_tons"], width=0.4, color=NAVY, label="Demand Carried (tons)")
            ax3b.bar(x + 0.2, by_vehicle["trips"], width=0.4, color=RED, label="Trips Dispatched")
            ax3.set_xticks(x); ax3.set_xticklabels(by_vehicle.index, rotation=0)
            apply_chart_style(ax3, xlabel="Vehicle", ylabel="Demand Carried (tons)", title="Demand vs Trips Dispatched, by Vehicle")
            ax3.title.set_color("black"); ax3.xaxis.label.set_color("black"); ax3.yaxis.label.set_color("black")
            ax3.tick_params(colors="black"); ax3b.tick_params(colors="black")
            ax3b.set_ylabel("Trips Dispatched"); ax3b.yaxis.label.set_color("black")
            lines1, labels1 = ax3.get_legend_handles_labels()
            lines2, labels2 = ax3b.get_legend_handles_labels()
            ax3.legend(lines1 + lines2, labels1 + labels2, loc="upper right", fontsize=8, frameon=False)
            plt.tight_layout()
            st.pyplot(fig3)
