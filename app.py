...
# ----------------------------------------------------------------
    ...
    st.markdown(
        ...
        A consolidated snapshot of all key logistics metrics discovered across
        Delivery Performance, Truck Utilization, Route Cost, Fuel Consumption,
        Store Replenishment, and Perishable Risk analyses.
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    # ============================================================
    # SAFE COLUMN DETECTION (SAME AS ALL PREVIOUS SECTIONS)
    # ============================================================
    on_time_col   = "Delivery_Status_[0]"
    delay_col     = "Delay_Minutes"
    vehicle_col   = "Vehicle_ID"   if "Vehicle_ID"   in df.columns else None
    ...
    # ============================================================
    # CHART ROW 1 — Delivery Status + Truck Utilization
    # ============================================================
    col1, col2 = st.columns(2)
    ...
    with col1:
        ...
        ax1.bar(["On-Time", "Late / Delayed"],
                [on_time_count, late_count],
                color=[BAR_BLUE, "#EF4444"])
    ...
    with col2:
        ...
        if util_col:
            ...
            ax2.bar(["≥70% (Efficient)", "<70% (Under-utilized)"],
                    [above_70, below_70],
                    color=[BAR_BLUE, "#EF4444"])
    ...
    # ============================================================
    # CHART ROW 2 — Top Cost Routes + Fuel Trend
    # ============================================================
    col3, col4 = st.columns(2)
    ...
    with col3:
        ...
        if route_col and cost_col:
            top_routes = (df.groupby(route_col)[cost_col]
                          .sum()
                          .sort_values(ascending=False)
                          .head(10))
    ...
    with col4:
        ...
        if date_col and fuel_col:
            df_temp          = df.copy()
            df_temp[date_col] = pd.to_datetime(df_temp[date_col], errors="coerce")
            fuel_trend       = (df_temp.groupby(df_temp[date_col].dt.to_period("M"))[fuel_col]
                                .sum().sort_index())
    ...
    # ============================================================
    # CHART ROW 3 — Store Delay + Delay Distribution
    # ============================================================
    ...
    with col5:
        ...
        if store_col and delay_col:
            store_delay = (df.groupby(store_col)[delay_col]
                           .mean()
                           .sort_values(ascending=False)
                           .head(10))
    ...
    st.markdown(
        ...
        <ul>
            <li>Dataset contains <b>{df.shape[0]:,} rows and {df.shape[1]} columns</b> — covering
                deliveries, routes, trucks, stores, fuel, cost, and temperature breach data.</li>
            <li>Overall data quality score: <b>{quality_score:.1f} / 100</b> — dataset is
                <b>model-ready</b> after preprocessing.</li>
            <li>Core identifiers (Route_ID, Vehicle_ID, Store_ID, Dispatch_Time) are
                <b>consistent and validated</b>.</li>
        </ul>
        ...
        <ul>
            ...
            <li><b>{pct_late:.1f}%</b> of all deliveries are delayed, with an average delay
                of <b>{avg_delay:.1f} minutes</b>.</li>
            <li>Certain routes and stores consistently underperform — they are
                <b>primary targets for route re-planning</b>.</li>
            <li>Monthly delivery volume shows seasonal patterns requiring
                <b>proactive fleet scheduling</b>.</li>
        </ul>
        ...
        <ul>
            ...
            <li><b>{pct_underutil:.1f}%</b> of all truck trips run below 70% capacity,
                representing direct <b>fuel and cost wastage</b>.</li>
            <li>Load consolidation and smarter dispatch can immediately reduce
                operational cost per delivery.</li>
        </ul>
        ...
        <ul>
            ...
            <li>A small number of routes contribute disproportionately to total cost —
                these are <b>candidates for route consolidation or re-sequencing</b>.</li>
            <li>Cost per KM analysis reveals routes with poor efficiency relative
                to distance covered.</li>
        </ul>

        <h4>Fuel Consumption</h4>
        <ul>
            <li>Total fuel cost across all deliveries: <b>${total_fuel:,.2f}</b>,
                averaging <b>${avg_fuel:,.2f}</b> per trip.</li>
            <li>Fuel cost is concentrated in high-distance, high-stop routes.</li>
            <li>A positive relationship between load weight and fuel cost confirms that
                <b>better load planning directly reduces fuel spend</b>.</li>
        </ul>

        <h4>Store Replenishment</h4>
        <ul>
            <li><b>{total_stores} unique stores</b> are served across the logistics network.</li>
            <li>Store <b>{worst_store_delay}</b> has the highest average delivery delay —
                requiring priority scheduling and dedicated route assignment.</li>
            <li>Replenishment frequency is uneven — some stores are chronically
                under-served, increasing stockout risk.</li>
        </ul>

        <h4>Perishable & Spoilage Risk</h4>
        <ul>
            <li>Temperature breach rate across all deliveries: <b>{breach_rate:.1f}%</b> —
            ...
            <li>Routes with high delays are also the routes with the
                <b>highest spoilage risk</b> — confirming that faster delivery directly
                reduces perishable loss.</li>
        </ul>

        <h4>Final Takeaway</h4>
        <ul>
            <li>The dataset is <b>clean, complete, and ready for ML-based
                route optimization modelling</b>.</li>
            <li>Key inefficiencies are clearly identified across delivery timing,
                truck utilization, fuel cost, and store replenishment.</li>
            <li>Route optimization at the
                <b>Route × Truck × Store × Driver × Time</b> level will directly
                address all identified pain points.</li>
            <li>EDA strongly supports downstream use cases in
                <b>route optimization, load planning, delivery scheduling,
                and spoilage reduction</b>.</li>
        </ul>
